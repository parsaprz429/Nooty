#!/usr/bin/env bash
# install.sh – Smart installer for NootyCLI v0.2.1 (macOS / Linux / WSL)
set -e
...
progress_bar() {
    ...
    for ((i=0;i<=steps;i++)); do
        ...
        printf "\r["; for ((j=0;j<filled;j++)); do printf "#"; done
        for ((j=0;j<empty;j++)); do printf "."; done; printf "] %d%%" $pct
...
echo -e "   Then: ${CYAN}/config${NC} , ${CYAN}/help${NC}"
